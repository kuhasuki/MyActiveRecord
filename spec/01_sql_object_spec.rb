...
describe SQLObject do
  ...
  describe '::finalize!' do
    it '::finalize! creates getter methods for each column' do
      c = Cat.new
    ...
    it '::finalize! created getter methods read from attributes hash' do
      c = Cat.new
      c.instance_variable_set(:@attributes, {name: "Nick Diaz"})
      expect(c.name).to eq 'Nick Diaz'
    end

    it '::finalize! creates setter methods for each column' do
      c = Cat.new
      c.name = "Nick Diaz"
      c.id = 209
      c.owner_id = 2
    ...
    it '::finalize! created setter methods use attributes hash to store data' do
      c = Cat.new
      c.name = "Nick Diaz"
